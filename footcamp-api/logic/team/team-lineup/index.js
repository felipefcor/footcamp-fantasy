...
module.exports = function (id, leagueId, teamId) {
  ...
  return (async () => {
    ...
    const players = await Promise.all(team.players.map((id_player) =>
    ...
    const goalkeepers = players.filter(player => player.position === 1).splice(0, 1)

    const defenders = players.filter(player => player.position === 2).splice(0, 4)
    const midfielders = players.filter(player => player.position === 3).splice(0, 4)
    const strikers = players.filter(player => player.position === 4).splice(0, 2)

    // extract the id of the players

    let idsGoalkeeper = []; let idsDefender = []; let idsMidfielder = []; let idsStriker = []
    ...
    const lineups = [...idsGoalkeeper, ...idsDefender, ...idsMidfielder, ...idsStriker]
